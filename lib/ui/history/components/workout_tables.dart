...
import 'package:workout_analyzer/ui/util.dart';

Widget? workoutTables(Workout workout, BuildContext context, {Set<String>? id, double spacing = 0.0}){

  final cardio = cardios(workout, context, id);
  final lift = exercises(workout, context, id);

  if(cardio == null && lift == null) return null;
...
Widget? exercises(Workout workout, BuildContext context, Set<String>? id){

  Set<String> exercisesIds = workout.exercises.map((e) => e.id).toSet();

  if(id != null){
    exercisesIds = exercisesIds.intersection(id);
  }

  if(exercisesIds.isEmpty) return null;
...
Widget? cardios(Workout workout, BuildContext context, Set<String> ? id){

  Set<String> cardioIds = workout.cardio.map((e) => e.id).toSet();

  if(id != null){
    cardioIds = cardioIds.intersection(id);
  }

  if(cardioIds.isEmpty) return null;
